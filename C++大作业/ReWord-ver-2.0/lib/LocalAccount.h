...
public:
    ...
    virtual void set_CRL(){return ;}
    virtual int get_CRL(){return 0;}
    virtual void set_CRE(){return ;}
    virtual int get_CRE(){return 0;}
    virtual void set_CRP(){return ;}
    virtual int get_CRP(){return 0;}
    virtual void set_ERL(){return ;}
    virtual int get_ERL(){return 0;}
    virtual void set_ERN(){return ;}
    virtual int get_ERN(){return 0;}
    // 我也不知道为什么下面3个虚函数能正常被重写但上面几个不行
...
public:
    Challenger(string new_name, string pwd, int flag=0,
        ...
        set_level(level);
        set_nop(nop);
        CRL = getRank("./Datas/CRL.csv", this->get_name());
        CRP = getRank("./Datas/CRP.csv", this->get_name());
        CRE = getRank("./Datas/CRE.csv", this->get_name());
    }
        ~Challenger(){}
    // 设置挑战者等级排名
    void Challenger::set_CRL(){this->CRL = getRank("./Datas/CRL.csv", this->get_name());}
    // 获取挑战者等级排名
    int Challenger::get_CRL(){return this->CRL;}
    // 设置挑战者经验排名
    void Challenger::set_CRE(){this->CRE = getRank("./Datas/CRE.csv", this->get_name());}
    // 获取挑战者经验排名
    int Challenger::get_CRE(){return this->CRE;}
    // 设置挑战者关卡数排名
    void Challenger::set_CRP(){this->CRP = getRank("./Datas/CRP.csv", this->get_name());}
    // 获取挑战者关卡数排名
...
public:
    ...
    Examiner(string new_name, string pwd, int flag=1,
        ...
        set_level(level);
        set_nop(nop);
        ERL = getRank("./Datas/ERL.csv", this->get_name());
        ERN = getRank("./Datas/ERN.csv", this->get_name());
    }
    ~Examiner(){}
    // 设置出题者等级排名
    void set_ERL(){this->ERL = getRank("./Datas/ERL.csv", this->get_name());}
    // 获取出题者等级排名
    int get_ERL(){return this->ERL;}
    // 设置出题者出题数排名
    void set_ERN(){this->ERN = getRank("./Datas/ERN.csv", this->get_name());}
    // 获取出题者出题数排名
